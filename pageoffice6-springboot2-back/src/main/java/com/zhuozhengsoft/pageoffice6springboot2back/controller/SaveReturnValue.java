 ...
 */
@RestController
@RequestMapping(value = "/basic/SaveReturnValue")
public class SaveReturnValue {
    ...
    public String word(HttpServletRequest request) {
         ...
         */
        poCtrl.webOpen("/doc/basic/SaveReturnValue/test.docx", OpenModeType.docNormalEdit, "张三");
        return  poCtrl.getHtmlCode();
    }
    @RequestMapping("/save")
    public void save(HttpServletRequest request, HttpServletResponse response) {
        FileSaver fs = new FileSaver(request, response);
        fs.saveToFile(dir + "basic/SaveReturnValue/" + fs.getFileName());
        //setCustomSaveResult方法的参数是一个字符串类型，甚至可以是json字符串
