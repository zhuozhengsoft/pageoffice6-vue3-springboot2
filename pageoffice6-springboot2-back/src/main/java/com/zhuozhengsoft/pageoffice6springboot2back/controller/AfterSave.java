 ...
 */
@RestController
@RequestMapping(value = "/basic/AfterSave")
public class AfterSave {
    ...
    public String word(HttpServletRequest request) {
         ...
         */
        poCtrl.webOpen("/doc/basic/AfterSave/test.docx", OpenModeType.docNormalEdit, "张三");
        return  poCtrl.getHtmlCode();
    }
    @RequestMapping("/save")
    public void save(HttpServletRequest request, HttpServletResponse response) {
        FileSaver fs = new FileSaver(request, response);
        fs.saveToFile(dir + "basic/AfterSave/" + fs.getFileName());
        fs.close();
    }
