	...
	import axios from 'axios';
	  export default{
	    ...
	    created: function(){
	      ...
	      axios.post("/api/word/InsertSeal/BatchAddSeal/index").then((response) => {
	    ...
	    methods:{
			editWord(id){
				...
				axios.post("/api/word/InsertSeal/BatchAddSeal/Edit?id="+id).then((response) => {
		  ...
		  getID() {
			  ...
			  //第一次自刷新
			  document.getElementById("iframe1").src = axios.post("/api/word/InsertSeal/BatchAddSeal/AddSeal?id="+this.str[this.count]).then((response) => {
														this.poHtmlCode = response.data;
