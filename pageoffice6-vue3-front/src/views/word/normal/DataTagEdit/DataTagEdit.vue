	...
	import axios from 'axios';
	  export default{
	    ...
	    created: function(){
	      ...
	      axios.post("/api/word/DataTagEdit/Word").then((response) => {
	    ...
	    methods:{
			...
			OnPageOfficeCtrlInit() {
				...
				pageofficectrl.AddCustomToolButton("定义数据标签", "ShowDefineDataTags()", 3);
			},
		  ...
		  ShowDefineDataTags() {
			  pageofficectrl.ShowHtmlModelessDialog("DataTagDlg", "parameter=xx", "left=300px;top=390px;width=430px;height=410px;");
  
		  },
		  
		  //获取后台定义的Tag 字符串
		  getTagNames() {
			  var tagNames = pageofficectrl.DefineTagNames;
			  return tagNames;
		  },
		  //定位Tag
		  locateTag(tagName) {
			  var appSlt = pageofficectrl.Document.Application.Selection;
			  var bFind = false;
			  //appSlt.HomeKey(6);
		  ...
		  addTag(tagName) {
			  try {
				  var tmpRange = pageofficectrl.Document.Application.Selection.Range;
